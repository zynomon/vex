...
licenses: [APACHE20]
deps: {
  qt6-core: { origin: devel/qt6-core, version: 6.8.2 }
  qt6-gui: { origin: x11-toolkits/qt6-gui, version: 6.8.2 }
  qt6-widgets: { origin: x11-toolkits/qt6-widgets, version: 6.8.2 }
}
EOF
...
cd "${PKG_ROOT}"
PKG_FILE="${PKG_OUT}/${PKG_NAME}_${PKG_VERSION}_${PKG_RELEASE}_freebsd.pkg"
pkg create -r . -o "${PKG_OUT}" -m .

find "${PKG_OUT}" -name "*.pkg" -not -name "*_freebsd.pkg" -exec bash -c '
    for file; do
        newname="${PKG_OUT}/${PKG_NAME}_${PKG_VERSION}_${PKG_RELEASE}_freebsd.pkg"
        mv "$file" "$newname"
